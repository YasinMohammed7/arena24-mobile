...
export const getCompleteRegistrationSchema = () => z
  .object({
    first_name: z
      .string()
    ...
    email: z
      .string()
      .min(1, { message: i18n.t('validation.emailAddress') })
      .email({ message: i18n.t('validation.emailAddressValid') }),
    password: z
      .string()
      .min(1, { message: i18n.t('validation.passwordRequired') })
  ...
  phone: z
    ...
    .refine(
      (val) => {
        ...
        return (
          /^\d+$/.test(restOfNumber) &&
          restOfNumber.length >= 10 &&
          restOfNumber.length <= 20
        );
      },
      { message: i18n.t('validation.phoneInvalid') }
    ),
...
export const getResetPasswordSchema = () => z
  .object({
    password: z
      .string()
      .min(1, { message: i18n.t('validation.passwordRequired') })
